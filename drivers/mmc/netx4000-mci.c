...
{
	struct netx4000_mmc_priv *priv = dev_get_priv(dev);
	struct netx4000_mmc_plat *plat = dev_get_platdata(dev);
...
{
	struct netx4000_mmc_priv *priv = dev->priv;
	fdt_addr_t addr;
...
U_BOOT_DRIVER(netx4000_mci) = {
	...
	.ofdata_to_platdata = netx4000_mmc_ofdata_to_platdata,
	.bind = netx4000_mmc_bind,
	.priv_auto_alloc_size = sizeof(struct netx4000_mmc_priv),
	.platdata_auto_alloc_size = sizeof(struct netx4000_mmc_plat),
};
